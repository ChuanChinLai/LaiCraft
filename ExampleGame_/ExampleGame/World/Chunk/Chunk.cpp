...
void LaiEngine::Chunk::SubmitDataToBeRendered(MainRenderer * renderer)
{
	for (auto& chunkSection : m_chunkSections)
	{
		chunkSection->SubmitDataToBeRendered(renderer);
	}
}
...
void LaiEngine::Chunk::SetBlock(int x, int y, int z, BlockType type)
{
	//if (!IsValid(x, y, z))
	//	return;
	//const int bY = y % CHUNK_SIZE;
...
void LaiEngine::Chunk::AddSectionsBlockTarget(int blockY)
{
	int index = blockY / CHUNK_SIZE;
	AddSectionsIndexTarget(index);
}

void LaiEngine::Chunk::AddSectionsIndexTarget(int index)
{
